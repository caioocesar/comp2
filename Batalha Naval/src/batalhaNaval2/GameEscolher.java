...
public class GameEscolher extends Game2 {
	...
	public GameEscolher(){
		...
		comecarJogo();
		//Testar onde estão os barcos u.u
		for(int i=0; i<10;i++){
			for(int j=0; j<10;j++){	
				tabuleiroJogador[i][j].addActionListener(this);
				if(tabuleiroJogador[i][j].getTemBarco() == 1){
					tabuleiroJogador[i][j].setBackground(Color.BLACK);
				}
			}
		}
		//teste posição dos barcos
		for(int i=0; i<10;i++){
			for(int j=0; j<10;j++){	
				if(tabuleiroComputador[i][j].getTemBarco() == 1){
					tabuleiroComputador[i][j].setBackground(Color.RED);
				}
			}
		}
		...
		for(int m=0; m<10; m++){
			for(int n=0; n<10; n++){
				int i=m, j=n;
				tabuleiroJogador[m][n].addActionListener(new ActionListener(){
	...
	public void posicionarBArcos(int tamanho) {
